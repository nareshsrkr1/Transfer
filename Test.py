...
updateFieldVisibilityAndAccessibility: async function (executionContext) {
    ...
    try {
        ...
        var isDirty = formContext.data.entity.getIsDirty(); // Check if there are changes

        if (isSaveEvent && statusPAA2 == 100000004 && hmDecision == null && isDirty) {
            if (!isConfirmedSave) {
        ...
        // --- Your existing field visibility logic ---
            ...
            if (careHomeID != null) {
                await Xrm.WebApi.retrieveRecord("bupa_bupacarehome", careHomeID, "?$select=bupa_homecategory")
                    .then(function (result) {
                        ...
                        if (category != null) {
                            // Apply your visibility/required logic here based on category
                        }
                    })
